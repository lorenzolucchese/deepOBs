...
#            print(e)
    ...
    for k in range(5):
        ...
        checkpoint_filepath = './model_weights/deepLOB_weights_FI2010/weights' + str(horizon)
...
#                    epochs=50, verbose=1, callbacks=[model_checkpoint_callback, early_stopping])
        ...
        # avoid RAM issues
        pred = deeplob.predict(generator.flow(testX_CNN, batch_size=32, shuffle=False), steps=len(testX_CNN) // 32, verbose=0)

        print("Prediction horizon:", horizon, "order book updates.\n")
