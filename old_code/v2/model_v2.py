...
from data_generators_v2 import CustomDataGenerator
...
class deepLOB:
    ...
    def evaluate_model(self, load_weights_filepath, eval_set = "test"):
        ...
        if self.data in ["FI2010", "simulated"]:
            ...
            evalY = eval_data[self.Y][:, self.horizon, ...]
        if self.data == "LOBSTER":
            evalY = np.zeros(predY.shape)
            eval_files = os.listdir(os.path.join(self.data_dir, eval_set))
            index = 0
            for file in eval_files:
                with np.load(os.path.join(self.data_dir, eval_set, file)) as data:
